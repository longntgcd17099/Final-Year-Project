...
public class ContentWorkOutActivity extends AppCompatActivity
implements View.OnClickListener, UserView {
    ...
    private void Init() {
        ...
        intent=getIntent();
         workOutModel= (WorkOutModel) intent.getSerializableExtra("WorkOut");



        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle(workOutModel.getName());
        ...
        txtnoidung.setText(Html.fromHtml(workOutModel.getNoidung()));
//        lottieAnimationView.setAnimationFromUrl(workOutModel.getLink());
...
//        lottieAnimationView.setSpeed(1.2f);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnbatdau: DiaLogInputWeight();break;
                ...
                if(editweight.length()>0){
                    try{
                        ...
                        if(weight>0){
                            ...
                            Toast.makeText(ContentWorkOutActivity.this, "Not ", Toast.LENGTH_SHORT).show();
                        }


                    }catch (Exception e){
                        Toast.makeText(ContentWorkOutActivity.this, "Input Number", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
    ...
    private void Practice(int checked) {
        ...
        if(checked==0){
            ...
            userPreSenter.HandleInsertUser(workOutModel.getCategories(),workOutModel.getName(),timer,simpleDateFormat.format(calendar.getTime()),"Excersie");
        }
